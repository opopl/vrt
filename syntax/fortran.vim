" Vim syntax file
" Language:	Fortran95 (and Fortran90, Fortran77, F and elf90) {{{1
" Version:	0.88
...
" let b:fortran_dialect = fortran_dialect if set correctly by user {{{1
...
" fortran_dialect not set or set incorrectly by user, {{{1
...
"}}}1
" Choose between fixed and free source form if this hasn't been done yet {{{1
...
"Catch errors caused by too many right parentheses {{{1
...
"If tabs are allowed then the left margin checks do not work {{{1
...
if (b:fortran_fixed_source == 1)
  if !exists("fortran_have_tabs")
    ...
    syn match fortranSerialNumber	excludenl "^.\{73,}$"lc=255
